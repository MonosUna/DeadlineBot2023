...
deadlines = dict()
"""Апдейт словаря дедлайнов, из сущесвующего текстовика"""
...
while data != "":
    ...
    prior = int(spl[k - 2])
    diff = int(spl[k - 1])  
    if id not in deadlines:
        deadlines[id] = []
    """Прошедшие дедлайны не добавляем"""
    now = datetime.datetime.now()
    lst = deadline.parser(datetm)
    date = datetime.datetime(lst[0], lst[1], lst[2], lst[3], lst[4])
    if date < now:
        continue

    deadlines[id].append(deadline.Deadline(datetm, name, prior, diff))
    data = str(f.readline())
f.close()
...
def update_data_base():
    ...
    for id in deadlines:
        for ddline in deadlines[id]:
            f.write(str(id) + " " + str(ddline.date) + " " + ddline.name + " " + str(ddline.priority) + " " + str(ddline.difficult) + "\n")
    f.close()
...
@dp.message()
async def UB(message: types.Message, state: FSMContext):
    await message.answer(Messages.UB, reply_markup=types.ReplyKeyboardRemove())
